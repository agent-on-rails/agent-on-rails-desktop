...
echo "Next: commit website public/downloads + pin, push, then HaloRT sync:"
echo "  cd ../agent-on-rails-website && bash scripts/deploy-image.sh  # if not already"
echo "  git push && bash scripts/sync-argo.sh"
echo "(sync-argo.sh uses ~/.kube/halort-platform/config — not default kubectl context.)"
